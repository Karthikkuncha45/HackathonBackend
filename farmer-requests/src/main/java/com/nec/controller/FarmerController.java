 ...
 // Get all loans for a specific farmer
    ...
    public ResponseEntity<Map<String, String>> createCheckoutSession(@RequestBody Map<String, Object> payload) {
        try {
            ...
            // ✅ Update the first loan record for this email
            ...
            loan.setAmount(Math.max(newAmount, 0)); // Avoid negative balance
            loanRepo.save(loan);

            // ✅ Create Stripe checkout session
            SessionCreateParams params = SessionCreateParams.builder()
                    .setMode(SessionCreateParams.Mode.PAYMENT)
                    .setSuccessUrl("http://localhost:5173/farmer_dashboard?session_id={CHECKOUT_SESSION_ID}&email=" + email)
                    .setCancelUrl("http://localhost:5173/farmer_dashboard/cancel")
                    .addLineItem(
